exports.getWeights = function (age, gender, alcoholConsumption, averageSleepDuration, sleepQuality) {
    if (age < 0 || age > 120) {
    ...
    if (averageSleepDuration === undefined || typeof averageSleepDuration !== 'number' || averageSleepDuration < 0) {
        throw new Error('Invalid average sleep duration. Sleep duration must be a non-negative number.');
    }

    if (sleepQuality === undefined || typeof sleepQuality !== 'number' || sleepQuality < 0 || sleepQuality > 100) {
        throw new Error('Invalid sleep quality. Sleep quality must be a number between 0 and 100.');
    }
